...
import org.camunda.bpm.modeler.ui.property.tabs.builder.ManualTaskPropertiesBuilder;
 ...
 */
public class GeneralTabCompositeFactory extends
		AbstractTabCompositeFactory<BaseElement> {

	public GeneralTabCompositeFactory(final GFPropertySection section,
	...
	public Composite createCompositeForBusinessObject(
			final BaseElement businessObject) {
		createIdField(businessObject);
		// Don't add a name field to a sequence flow .. that only confuses
		// people...
	...
	private void createGatewayComposite(final Gateway gateway) {
		if (gateway instanceof ExclusiveGateway) {
			new DecisionGatewayPropertiesBuilder(parent, section, gateway)
					.create();
		}
	...
	private void createSequenceFlowComposite(final SequenceFlow sequenceFlow) {
		new SequenceFlowPropertiesBuilder(parent, section, sequenceFlow)
				.create();
	}

	private void createTextAnnotationComposite(final TextAnnotation annotation) {
		new TextAnnotationPropertiesBuilder(parent, section, annotation)
				.create();
	}
	...
	private void createActivityComposite(final Activity activity) {
		// if (!(activity instanceof ManualTask) && !(activity instanceof
		// AdHocSubProcess)) {
		// new ActivityPropertiesBuilder(parent, section, activity).create();
		// }

		if (activity instanceof CallActivity) {
			new CallActivityPropertiesBuilder(parent, section,
					(CallActivity) activity).create();
		}

		if (activity instanceof SubProcess) {
			new SubProcessPropertiesBuilder(parent, section,
					(SubProcess) activity).create();
		}
	...
	private boolean isTimerEvent(final CatchEvent e) {
		EventDefinition timerEventDefinition = ModelUtil.getEventDefinition(e,
	...
	private void createTaskComposite(final Task task) {
		...
		// new ServiceTypeControlsPropertiesBuilder(parent, section,
		// task).create();
		// new ServiceTaskPropertiesBuilder(parent, section, task).create();
		// }

		if (task instanceof UserTask) {
			new UserTaskPropertiesBuilder(parent, section, (UserTask) task)
					.create();
		} else

		// if (task instanceof ScriptTask) {
		// new ScriptTaskPropertiesBuilder(parent, section, (ScriptTask)
		// task).create();
		// } else
		//
		if (task instanceof ServiceTask) {
			new AdvancedServiceTaskPropertiesBuilder(parent, section, task)
					.create();
		}

		if (task instanceof ManualTask) {
			new ManualTaskPropertiesBuilder(parent, section, (ManualTask) task)
					.create();
		}
	}
	...
	private void createEventComposite(final Event event) {
		if (event instanceof StartEvent) {
			new StartEventPropertiesBuilder(parent, section, (StartEvent) event)
					.create();
		}
		...
		if (event instanceof IntermediateThrowEvent
			...
			EventDefinition messageEventDefinition = ModelUtil
					.getEventDefinition(event, MessageEventDefinition.class);
			if (messageEventDefinition != null) {
				new ServiceTypeControlsPropertiesBuilder(parent, section,
						messageEventDefinition).create();
				new ActivityPropertiesBuilder(parent, section,
						messageEventDefinition).create();
			}
		}
	}
	...
	private void createIdField(final BaseElement baseElement) {
		if (baseElement instanceof Process) {
			new ProcessIdPropertyBuilder(parent, section, (Process) baseElement)
					.create();
		} else {
	...
	private void createDocumentationField(final BaseElement baseElement) {
		new DocumentationPropertiesBuilder(parent, section, baseElement)
				.create();
	}

	private void createIsForCompensationFlag(final BaseElement baseElement) {
		new IsForCompensationPropertiesBuilder(parent, section, baseElement)
				.create();
	}
}
